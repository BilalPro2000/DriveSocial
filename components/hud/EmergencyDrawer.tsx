...
export function EmergencyDrawer({ onClose }: EmergencyDrawerProps) {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-0 left-0 w-full bg-[#121824]/95 backdrop-blur-2xl rounded-t-3xl border-t border-red-500/30 shadow-[0_-10px_40px_rgba(255,59,48,0.2)] z-[100] pb-10"
    >
      ...
      <div className="px-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => handleCall('911')} className="bg-red-500 hover:bg-red-600 active:scale-[0.98] transition-all rounded-2xl p-4 flex flex-col items-center justify-center gap-2 shadow-[0_0_15px_rgba(255,59,48,0.4)]">
            <Phone className="w-8 h-8 text-white" />
            <span className="font-bold text-white uppercase tracking-wider">Call 911</span>
          </button>
          
          <button onClick={() => handleCall('Roadside Assistance')} className="bg-white/10 hover:bg-white/20 active:scale-[0.98] transition-all rounded-2xl p-4 flex flex-col items-center justify-center gap-2 border border-white/10">
            <Wrench className="w-8 h-8 text-white" />
            <span className="font-bold text-white uppercase tracking-wider">Tow Truck</span>
          </button>

          <button onClick={handleHazards} className={`col-span-2 active:scale-[0.98] transition-all rounded-2xl p-4 flex items-center justify-center gap-3 border ${hazardsOn ? 'bg-amber-500 text-white border-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.5)] animate-pulse' : 'bg-white/10 text-white border-white/10'}`}>
